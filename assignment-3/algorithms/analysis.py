...
def d_compute_asymptotic_constant(
    ...
    for graph in graphs:
        w, run_time = r_measure_run_time(alg, graph, num_calls)
        n = graph.get_n()
        ...
        weights.append(w)
        graphs_dimensions.append(n)
        c_estimates.append(round(run_time / asymptotic_behaviour(n), 3))
...
def plot_run_vs_discovery(run_times,
    discovery_times,
    graphs_dimensions,
    title,
):
...
def plot_complexity(
    ...
    x = graphs_dimensions
    reference_z = [C * asymptotic_behaviour(n) for n in graphs_dimensions]
