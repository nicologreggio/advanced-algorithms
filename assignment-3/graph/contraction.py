...
def get_nth_vertex(d, nth: int) -> Vertex:  # d is a list of keys (list[Vertex]?)
...
def binary_search(C: List[int], r: int) -> int:
...
def random_select(C: List[int], d) -> Vertex:  # d is a list of keys
    r = randrange(0, C[-1])
    e = binary_search(C, r)
    return get_nth_vertex(d, e - 1)


def edge_select(g: Graph) -> Tuple[Vertex, Vertex]:
